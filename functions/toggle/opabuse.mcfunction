...
execute @s[tag=ownerstatus,scores={OPAM=1}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§eUnlimited §¶§b► §6Anti OP Abuse §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
#turn off
...
execute @s[tag=ownerstatus,scores={OPAM=2}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§eUnlimited §¶§b► §6Anti OP Abuse §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
execute @s[tag=ownerstatus,scores={OPAM=2}] ~~~ scoreboard players set opamtoggledummy OPAM 0
#Deny Nonstaff
execute @s[tag=!ownerstatus] ~~~ tellraw @s {"rawtext":[{"text":"§¶§eUnlimited §¶§b► Access §cDENIED§7! §bOnly Ownerstatus can use this command"}]}

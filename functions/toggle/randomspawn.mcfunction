...
execute @s[tag=staffstatus,scores={RSM=1}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§eUnlimited §¶§b► §6Random Spawn §bhas been toggled §2ON §bby §d"},{"selector":"@s"}]}
tellraw @s[tag=staffstatus,scores={has_xx=0,RSM=1}] {"rawtext":[{"text":"§¶§eUnlimited ► §6Gametest Features §7: §cNot Enabled §7|| §cWorldSpawn will randomize but worldborder will not teleport players to the randomized coords"}]}
#turn off
...
execute @s[tag=staffstatus,scores={RSM=2}] ~~~ tellraw @a[tag=staffstatus] {"rawtext":[{"text":"§¶§eUnlimited §¶§b► §6Random Spawn §bhas been toggled §cOFF §bby §d"},{"selector":"@s"}]}
